...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!body || typeof body !== 'object' || !['join', 'sync', 'replay', 'friends', 'public', 'robot', 'robot-setup'].includes(body.action)) return json({ error: 'Unknown action' }, 400);
    ...
    if (!player) {
      if (!['join', 'friends', 'public', 'robot', 'robot-setup'].includes(body.action)) return json({ error: 'Join a game first' }, 401);
    ...
    if (body.action === 'robot' || body.action === 'robot-setup' || (body.action === 'replay' && room?.robot_ms)) {
      ...
      if (body.roomId === room.id && ['finished', 'cancelled'].includes(room.status)) {
        ...
        if (room.ready1 && room.ready2 && !room.next_room) {
          const next = crypto.randomUUID();
          await db.prepare("INSERT INTO rooms(id,p1,p2,status,puzzle,created,start,friend_code) VALUES(?,?,?,'playing',?,?,?,?)").bind(next, room.p1, room.p2, await choosePuzzle(db, room.p1, room.p2 ?? room.p1), now, now + 4000, room.friend_code).run();
