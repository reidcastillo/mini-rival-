...
type Room = { id: string; p1: string; p2: string | null; status: string; puzzle: number; created: number; start: number | null; ended: number | null; winner: string | null; a1: string; a2: string; rev1: number; rev2: number; friend_code: string | null; ready1: boolean; ready2: boolean; next_room: string | null; robot_ms: number | null; robot_level: string | null; ruleset: 'classic' | 'rumble'; powers1: string; powers2: string };
const json = (data: unknown, status = 200) => Response.json(data, { status, headers: { 'Cache-Control': 'no-store' } });
const first = <T,>(db: Database, sql: string, ...args: (string | number | null)[]) => db.prepare(sql).bind(...args).first<T>();

const puzzleWords = puzzles.map((_, index) => {
  const details = puzzleDetails(index);
  return new Set([...details.public.across, ...details.public.down].map(clue => clue.cells.map(cell => details.solution[cell]).join('')));
});

async function choosePuzzle(db: Database, id: string, other = id) {
  const recent = await db.prepare("SELECT puzzle FROM rooms WHERE (p1 IN (?,?) OR p2 IN (?,?)) AND start IS NOT NULL ORDER BY created DESC LIMIT 1000").bind(id, other, id, other).all<{ puzzle: number }>();
  const excluded = new Set(recent.results.map(r => r.puzzle));
  const available = puzzles.map((_, i) => i).filter(i => i > 0 && !excluded.has(i));
  const choices = available.length ? available : puzzles.map((_, i) => i).filter(i => i > 0 && !recent.results.slice(0,20).some(r => r.puzzle === i));
  const recentWords = new Set(recent.results.slice(0,5).flatMap(r => [...(puzzleWords[r.puzzle] ?? [])]));
  const ranked = choices.map(index => ({index, tie: crypto.getRandomValues(new Uint32Array(1))[0], overlap: [...puzzleWords[index]].filter(word => recentWords.has(word)).length})).sort((a,b) => a.overlap-b.overlap || a.tie-b.tie);
  const shortlist = ranked.slice(0, Math.min(30,ranked.length));
  return shortlist[crypto.getRandomValues(new Uint32Array(1))[0] % shortlist.length].index;
}

async function join(db: Database, id: string, now: number, ruleset = 'classic') {
  ...
  await db.batch([
    ...
    db.prepare("UPDATE rooms SET p2=?, status='playing', start=? WHERE id=(SELECT r.id FROM rooms r JOIN players p ON p.id=r.p1 WHERE r.status='waiting' AND r.friend_code IS NULL AND r.robot_ms IS NULL AND r.ruleset=? AND r.p1<>? AND p.seen>=? ORDER BY r.created, r.id LIMIT 1) AND (SELECT room FROM players WHERE id=?) IS NULL").bind(id, now + 4000, ruleset, id, now - 45000, id),
    db.prepare("INSERT INTO rooms(id,p1,status,puzzle,created,ruleset) SELECT ?,?,'waiting',?,?,? WHERE (SELECT room FROM players WHERE id=?) IS NULL AND NOT EXISTS(SELECT 1 FROM rooms WHERE p2=? AND status='playing')").bind(room, id, puzzle, now, ruleset, id, id),
  ...
  const paired = await first<Room>(db, "SELECT * FROM rooms WHERE p2=? AND start=? AND status='playing' AND robot_ms IS NULL", id, now+4000);
  if (paired) await db.prepare('UPDATE rooms SET puzzle=? WHERE id=?').bind(await choosePuzzle(db,paired.p1,id),paired.id).run();
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!player) {
      if (!['join', 'friends', 'public', 'robot', 'robot-setup', 'classic', 'rumble'].includes(body.action)) return json({ error: 'Join a game first' }, 401);
    ...
    if (body.action === 'classic' || body.action === 'rumble') {
      ...
      if (!room?.friend_code || room.status !== 'waiting') {
        ...
        await db.prepare("INSERT INTO rooms(id,p1,status,puzzle,created,friend_code,ruleset) VALUES(?,?,'waiting',?,?,?,?)").bind(roomId, id, await choosePuzzle(db, id), now, code, room?.ruleset ?? 'classic').run();
        await db.prepare('UPDATE players SET room=? WHERE id=?').bind(roomId, id).run();
      }
    } else if (body.action === 'public') {
      await leave(db, id, room, now);
      await join(db, id, now, room?.ruleset ?? 'classic');
      ...
      if (invited.p1 !== id && invited.p2 !== id) {
        ...
        await leave(db, id, room, now);
        await db.prepare("UPDATE rooms SET p2=?, status='playing', start=?, puzzle=? WHERE id=? AND p2 IS NULL").bind(id, now + 4000, await choosePuzzle(db,invited.p1,id), invited.id).run();
      }
      ...
      if (body.roomId === room.id && ['finished', 'cancelled'].includes(room.status)) {
        ...
        if (room.ready1 && room.ready2 && !room.next_room) {
          const next = crypto.randomUUID();
          await db.prepare("INSERT INTO rooms(id,p1,p2,status,puzzle,created,start,friend_code,ruleset) VALUES(?,?,?,'playing',?,?,?,?,?)").bind(next, room.p1, room.p2, await choosePuzzle(db, room.p1, room.p2 ?? room.p1), now, now + 4000, room.friend_code, room.ruleset).run();
    ...
    room = (await first<Room>(db, 'SELECT * FROM rooms WHERE id=? FOR UPDATE', room.id))!;
    ...
    let incorrect = false;
    if (body.answers !== undefined && !frozen) {
      ...
      await db.prepare(`UPDATE rooms SET a${seat}=?, rev${seat}=? WHERE id=? AND status='finished' AND winner<>? AND rev${seat}<?`).bind(JSON.stringify(answers),body.revision,room.id,id,body.revision).run();
    }
    room = (await first<Room>(db, 'SELECT * FROM rooms WHERE id=?', room.id))!;
    ...
    let powers = readPowers(seat === 1 ? room.powers1 : room.powers2);
    if (room.ruleset === 'rumble' && room.status === 'playing' && (room.start ?? Infinity) <= now) {
      powers = earnPowers(powers,ownAnswers,details.solution);
      if (body.action === 'power') {
        if (body.roomId !== room.id) return json({error:'This power belongs to a different race.'},409);
        const power = body.power as Power, bit = powerBits[power];
        if (!(powers.earned & bit)) return json({error:'Reach the milestone to unlock this power.'},409);
        if (!(powers.used & bit)) {
          if (powers.frozenUntil > now) return json({error:'You are frozen. Wait for the ice to melt.'},409);
          powers.used |= bit;
          if (power === 'check') powers.checked = checkedLetters(ownAnswers,details.solution);
          else {
      ...
      await db.prepare(`UPDATE rooms SET powers${seat}=? WHERE id=?`).bind(JSON.stringify(powers),room.id).run();
    } else if (body.action === 'power') return json({error:'Powers are only available during a Rumble race.'},409);
    ...
    return json({ now: Date.now(), player: { name: player.name }, room: { id: room.id, ruleset: room.ruleset, rumble: room.ruleset === 'rumble' ? powers : null, mode: room.robot_ms ? 'robot' : room.friend_code ? 'friends' : 'public', robotDifficulty: room.robot_level, invite: room.friend_code, ready: seat === 1 ? room.ready1 : room.ready2, opponentReady: seat === 1 ? room.ready2 : room.ready1, status: room.status, start: room.start, ended: room.ended, won: room.winner === id, solved: details.solution.every((letter,i) => letter === '#' || ownAnswers[i] === letter), opponent: opponent ? { name: opponent.name, connected: !!room.robot_ms || opponent.seen >= now - 10000 } : null, progress: Array.from({ length: 25 }, (_, i) => !!otherAnswers[i]), answers: ownAnswers.length ? ownAnswers : Array(25).fill(''), revision: seat === 1 ? room.rev1 : room.rev2, puzzle: room.status === 'waiting' || (room.start ?? Infinity) > now ? null : details.public }, incorrect, leaderboard: leaderboard.results });
    });
  } catch (error) {
